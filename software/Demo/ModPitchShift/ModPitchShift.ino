...
const float pitch_shift_ratio = 1.5; 
AudioInfo info(48000, 2, 32);

I2SStream i2s;
NumberFormatConverterStreamT<int16_t, int32_t> out_conv(i2s);
CustomPitchShiftOutput<int16_t, CustomVariableSpeedRingBuffer<int16_t>> pitch_shifter(out_conv);
...
void loop() {
  pitch_shifter.buffer.setIncrement(sin((float) micros() / 1e6 * 3.0) * 0.6 + 1.0);
  copier.copy();
}

 ...
 * 
 * 1. **CustomVariableSpeedRingBufferSimple**: Basic implementation with potential artifacts
 *    - Fastest processing
 *    - Suitable for simple applications where quality is not critical
 *    - May produce audible artifacts during buffer overruns
 * 
 * 2. **CustomVariableSpeedRingBuffer180**: Advanced implementation with 180° phase offset
 ...
 * 
 * 3. **CustomVariableSpeedRingBuffer**: Premium implementation with interpolation
...
/**
 * @brief Configuration for CustomPitchShiftOutput
 * 
 ...
 */
struct CustomPitchShiftInfo : public AudioInfo {
  CustomPitchShiftInfo() {
    channels = 2;
    sample_rate = 44100;
    bits_per_sample = 16;
  }
 ...
 *          pointer overruns. For better quality, consider using CustomVariableSpeedRingBuffer180
 *          or CustomVariableSpeedRingBuffer.
 * 
 * @ingroup buffers
 * @tparam T The sample data type (typically int16_t or float)
 */
template <typename T>
class CustomVariableSpeedRingBufferSimple : public BaseBuffer<T> {
 public:
  /**
   * @brief Constructor
   ...
   */
  CustomVariableSpeedRingBufferSimple(int size = 0, float increment = 1.0) {
 ...
 */
template <typename T>
class CustomVariableSpeedRingBuffer180 : public BaseBuffer<T> {
 public:
  /**
   * @brief Constructor
   * @param size Initial buffer size (0 means no allocation yet)
   ...
   */
  CustomVariableSpeedRingBuffer180(int size = 0, float increment = 1.0) {
    setIncrement(increment);
    if (size > 0) resize(size);
  }
 ...
 */
template <typename T>
class CustomVariableSpeedRingBuffer : public BaseBuffer<T> {
 public:
  /**
   * @brief Constructor
   * @param size Initial buffer size (0 means no allocation yet)
   ...
   */
  CustomVariableSpeedRingBuffer(int size = 0, float increment = 1.0) {
    setIncrement(increment);
    if (size > 0) resize(size);
  }
 ...
 * - CustomVariableSpeedRingBufferSimple: Basic implementation with potential artifacts
 * - CustomVariableSpeedRingBuffer180: Uses 180° phase shifting to reduce artifacts  
 * - CustomVariableSpeedRingBuffer: Advanced implementation with interpolation and phase restoration
 * 
 ...
 * @tparam BufferT Buffer implementation type (one of the CustomVariableSpeedRingBuffer variants)
 */
template <typename T, class BufferT>
class CustomPitchShiftOutput : public AudioOutput {
 public:
  /**
   * @brief Constructor
   * @param out Reference to the output stream where processed audio will be written
   */
  CustomPitchShiftOutput(Print &out) { p_out = &out; }
  ...
  size_t write(const uint8_t *data, size_t len) override {
    LOGD("CustomPitchShiftOutput::write %d bytes", (int)len);
    if (!active) return 0;
    ...
    for (int j = 0; j < sample_count; j += channels) {
      ...
      LOGD("CustomPitchShiftOutput %f -> %d", value, (int)out_value);
